...
describe("ToolsTab", () => {
  ...
  it("should handle union string|null type inputs (like FastMCP optional parameters)", async () => {
    const unionTool = {
      name: "searchTool",
      description: "Search with optional category",
      inputSchema: {
        type: "object" as const,
        properties: {
          category: {
            anyOf: [{ type: "string" as const }, { type: "null" as const }],
            description: "Optional category parameter",
          },
        },
        required: [],
      },
    ...
    // Should call with the string value, not null
    expect(defaultProps.callTool).toHaveBeenCalledWith(unionTool.name, {
      category: "hello",
    });
  });

  it("should omit undefined optional parameters from tool calls", async () => {
    const unionTool = {
      name: "searchTool",
      description: "Search with optional category",
      inputSchema: {
    ...
    // User types something then deletes it, leaving empty
    fireEvent.change(input, { target: { value: "hello" } });
    ...
    // Should omit the empty parameter entirely
    expect(defaultProps.callTool).toHaveBeenCalledWith(unionTool.name, {});
  });
